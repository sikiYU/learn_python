...
#map  map()函数接收两个参数，一个是函数，一个是 Iterable，map 将传入的函数依次作用到序列的每个元素，并把结果作为新的Iterator 返回
def f(x):
	return x*x
L = map(f,range(1,10))
print(list(L))
...
def x(x,y):
	return x*10+y
print(reduce(x,[1,3,5,7,9,0]))

#利用 map()函数，把用户输入的不规范的英文名字，变为首字母大写，其他小写的规范名字。
def standerd_name(L):
	def step1(name):
		return name[:1].upper() + name[1:].lower()
	return list(map(step1,L))

print(standerd_name(['adam','LISA','barT']))
...
#filter 过滤序列,接收一个函数和一个序列,把传入的函数依次作用于每个元素，然后根据返回值是 True 还是 False 决定保留还是丢弃该元素


#sorted
L = sorted([36, 5, -12, 9, -21], key=abs)
print(L)
L = sorted(['a', 'B', 'Z', 'E'], key=str.lower)
print(L)
#反响排序
